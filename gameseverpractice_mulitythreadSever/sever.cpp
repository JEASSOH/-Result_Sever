...
// 
// 
//그리고 사실 주고받는 값을 이런식으로 처리해도 괜찮은지도 잘 모르겠다. 문제도 100퍼센트 이해하지 못한게 조금 크다
...
LPVOID ProcessClient(LPVOID arg)
{
    ...
    while (1) {
        ...
        // 받은 데이터 출력
        buf[retval] = '\0';
        printf("[TCP/%s:%d] 받은값 %s [쉐어값]%d\n", inet_ntoa(clientaddr.sin_addr),
            ntohs(clientaddr.sin_port), buf, share);

        retval2 = recv(client_sock, buf2, BUFSIZE, 0);
        if (retval == SOCKET_ERROR) {
            err_display("recv()");
            break;
        }
        ...
        buf2[retval2] = '\0';
        //share = share + atof(buf2);
        share = share + 10;
        sprintf(buf2, "%d", share);
        printf("[TCP/%s:%d] 받은값 %s [쉐어값]%d\n", inet_ntoa(clientaddr.sin_addr),
            ntohs(clientaddr.sin_port), buf2, share);
        ...
        retval2 = send(client_sock, buf2, retval2, 0);
        if (retval2 == SOCKET_ERROR) {
            err_display("send()");
            break;
        }
        printf("[TCP/%s:%d] 보낸값 %s 보낸값[쉐어값]%d\n", inet_ntoa(clientaddr.sin_addr),
            ntohs(clientaddr.sin_port), buf2, share);
    }
